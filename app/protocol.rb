...
class Protocol
  ...
  def touch_remote_by_score_header(score_header)
    ...
    score = Zold::Score.load score_header
  ...
  def add_response_headers(headers)
    add_http_headers headers, reduce: 16, serializer: :to_s
  end

  private

  def add_http_headers(headers, reduce:, serializer:)
    headers[PROTOCOL_HEADER] = VERSION.to_s
    headers[VERSION_HEADER]  = Zoldy::VERSION.to_s

    headers[NETWORK_HEADER]  = Settings.network
    headers[SCORE_HEADER]    = Zoldy.app.scores_store.best.reduced(reduce).send(serializer)
